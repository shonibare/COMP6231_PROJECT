...
public class ReplicaManagerBackUp {
...
new Thread() {
	...
	private static String processsRequest(List request) {
		...
		if (no==1) {
			...
			case "MTL":
			{
				ServerInterface con = ServerInterfaceHelper.narrow(ncRef.resolve_str("MTL_Server"));
			...
			case "QUE":
			{
				ServerInterface con = ServerInterfaceHelper.narrow(ncRef.resolve_str("QUE_Server"));
				result = con.addEvent(managerId, eventId, eventType, capacity);
				storage.put(seq, result);
				break;
			}
			case "SHE":
			{
				ServerInterface con = ServerInterfaceHelper.narrow(ncRef.resolve_str("SHE_Server"));
				result = con.addEvent(managerId, eventId, eventType, capacity);
				storage.put(seq, result);
				break;
			}
