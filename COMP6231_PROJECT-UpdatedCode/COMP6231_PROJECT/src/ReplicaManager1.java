...
new Thread() {
	...
	private static String processsRequest(List request) {
		...
		if (no==1) {
			...
			case "MTL":
			{
			    ServerInterface server = (ServerInterface)ServerInterfaceHelper.narrow(ncRef.resolve_str("MTLServer"));
                
                result = server.addEvent(eventType, eventId, capacity);
			...
			{
			//	
				ServerInterface server = (ServerInterface)ServerInterfaceHelper.narrow(ncRef.resolve_str("QUEServer"));
			...
			case "SHE":
			{
			    ServerInterface server = (ServerInterface)ServerInterfaceHelper.narrow(ncRef.resolve_str("SHEServer"));
			...
			case "MTL":
			{
				ServerInterface server = ServerInterfaceHelper.narrow(ncRef.resolve_str("MTLServer"));
				result = server.removeEvent(eventId, eventType);
				storage.put(seq, result);
				break;
			}
			case "QUE":
			{
				ServerInterface server = ServerInterfaceHelper.narrow(ncRef.resolve_str("QUEServer"));
				result = server.removeEvent(eventId, eventType);
				storage.put(seq, result);
				break;
			}
			case "SHE":
			{
				ServerInterface server = ServerInterfaceHelper.narrow(ncRef.resolve_str("SHEServer"));
				result = server.removeEvent(eventId, eventType);
				storage.put(seq, result);
				break;
			}
			...
			String seq = (String) request.get(2);
				ServerInterface server = ServerInterfaceHelper.narrow(ncRef.resolve_str("SHEServer"));
				result = server.listEventAvailability(eventType);
				storage.put(seq, result);
			...
			{
				ServerInterface server = ServerInterfaceHelper.narrow(ncRef.resolve_str("MTLServer"));
				result = server.getBookingSchedule(customerId);
				storage.put(seq, result);
				break;
			}
			case "QUE":
