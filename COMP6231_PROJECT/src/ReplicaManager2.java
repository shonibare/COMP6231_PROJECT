...
public class ReplicaManager2 {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            System.out.println("This is replica manager 2.");
        ...
        try {
            if (no==1) {
                ...
                switch(func) {
                    case "MTL":
                    {
                        ServerInterface con = (ServerInterface) ServerInterfaceHelper.narrow(ncRef.resolve_str("MTL"));
                        result = con.addEvent(eventId, eventType, capacity, managerId);
                        storage.put(seq, result);
                        break;
                    }
                    case "QUE":
                    {
                        ServerInterface con = (ServerInterface) ServerInterfaceHelper.narrow(ncRef.resolve_str("QUE"));
                        result = con.addEvent(eventId, eventType, capacity, managerId);
                        storage.put(seq, result);
                        break;
                    }
                    case "SHE":
                    {
                        ServerInterface con = (ServerInterface) ServerInterfaceHelper.narrow(ncRef.resolve_str("SHE"));
                        result = con.addEvent(eventId, eventType, capacity, managerId);
                        storage.put(seq, result);
                        break;
                    }
                ...
                switch(func) {
                    ...
                    {
                        ServerInterface con = (ServerInterface) ServerInterfaceHelper.narrow(ncRef.resolve_str("MTL"));
                        result = con.removeEvent(eventId, eventType, managerId);
                        storage.put(seq, result);
                        break;
                    }
                    case "QUE":
